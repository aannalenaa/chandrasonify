...
from chandrasonify.config import (  # noqa: E402
    CIAO_BIN, SF2, VIDEO_CODEC, BROKEN_PRESETS, print_config
)
from chandrasonify.llm_config import (  # noqa: E402
    DUAL_BAND_THRESHOLD, ENERGY_BANDS, BAND_LABELS,
)
...
"""
LLM-assisted X-ray data processing pipeline.

Same pipeline structure as manual_code.py, but detection parameters,
sonification parameters, and band mode selection are decided by an LLM
based on observation metadata, SIMBAD context, and source characteristics.

1. activate ciao_venv (inside the macos distribution)
2. install all required packages
3. start the LLM server:
    llama-server -m Llama-3.1-8B-Instruct-Q8_K_M.gguf --port 8000
4. source /ciao_venv/bin/activate
6. source /Applications/ciao-4.18/bin/ciao.sh
7. python final_code/llm_code.py -o 22304 --fresh
"""
...
# — LLM Defaults ———————————————————————————————————————————————
FALLBACK_WAVDETECT_SCALES = "1 2 4 8"
FALLBACK_SIGNIFICANCE_THRESHOLD = 1e-6
FALLBACK_DURATION = 60.0
FALLBACK_PEAK_VOLUME_RANGE = (0.3, 1.0)
FALLBACK_STEREO_SPREAD = (0.3, 0.7)
FALLBACK_NOTE_LEN = 1.0
FALLBACK_SF_PRESET = 1
FALLBACK_MASTER_VOLUME = 0.6
FALLBACK_DUAL_BAND_THRESHOLD = 25

# Initialize LLM
llm: ChatOpenAI = ChatOpenAI(
    base_url="http://localhost:8000/v1",
    api_key=SecretStr("not-needed-here"),
    model="llama-3.1-8b",
    temperature=0.5,
    timeout=60.0
)
...
def invoke_llm_with_retry(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            return str(response.content) if \
                hasattr(response, 'content') else str(response)
...
def extract_json(response: str | Any) -> dict[Any, Any]:
    """Extract JSON from LLM response.

    Args:
        response (str | Any): Raw LLM response string or object with content.

    Returns:
        dict[Any, Any]: Parsed JSON object or empty dict on failure.
    """
    ...
    try:
        ...
        print(f"Could not parse JSON from LLM: {response[:150]}...")
        return {}
        ...
        if csv_path.exists():
            with open(csv_path, 'r') as f:
                ...
                for row in reader:
                    ...
                    otype_desc = row['description'].strip() \
...
def run_validity_checks(
    ...
    edge_mask = (
        (x < edge_margin) | (x > chip_size - edge_margin) |
        (y < edge_margin) | (y > chip_size - edge_margin)
    )
...
class PipelineManager:
    ...
    resume logic, deterministic step sequencing,
    and LLM-based parameter suggestion.
    """
    ...
    def __init__(self, workspace: str):
        ...
        Args:
            workspace (str): Absolute path to the workspace root directory
                containing EVT2 files and output subdirectories.
        """
    ...
    def suggest_detection_parameters(
            self,
            metadata: ObservationMetadata,
            previous_attempt: dict | None = None
    ) -> ProcessingParameters:
        """Suggest optimal wavdetect parameters via LLM reasoning.

        If previous_attempt is provided, the LLM can learn from
        suboptimal results and adapt. Validates LLM response and
        returns fallback defaults if parsing fails.

        Args:
            metadata (ObservationMetadata): Observation metadata including
                target name, exposure time, object type for context.
            previous_attempt (dict | None): Optional dict describing a
                previous detection attempt; LLM uses this for adaptive
                parameter adjustment.

        Returns:
            ProcessingParameters: Wavdetect scales, significance threshold,
                and reasoning explanation. Falls back to defaults if LLM
        ...
        if previous_attempt:
            adaptation_context = (
                ...
                "Please suggest better parameters "
                "if this attempt was suboptimal."
            )

        prompt = f"""You are an X-ray astronomy expert \
                optimizing source detection parameters.

                Observation: {metadata.target_name}
                Exposure: {metadata.exposure_time}s
                Object Type: {metadata.object_type or 'unknown'}
                Object Info: {metadata.object_info or 'no additional info'}
                ...
                {{"wavdetect_scales": "1 2 4 8 16", \
                "significance_threshold": 1e-6, "reasoning": "..."}}"""

        response = invoke_llm_with_retry(prompt)
        parsed = extract_json(response)

        try:
            adapter = TypeAdapter(ProcessingParameters)
            params = adapter.validate_python(parsed)
            return params
        except ValidationError as e:
            print(f"  LLM parameter validation failed: {e}")
            return ProcessingParameters(
                wavdetect_scales=FALLBACK_WAVDETECT_SCALES,
                significance_threshold=FALLBACK_SIGNIFICANCE_THRESHOLD,
                reasoning="Fallback defaults (LLM validation failed)"
            )

    def suggest_sonification_parameters(
            self,
            metadata: ObservationMetadata
    ) -> SonificationConfig:
        """Suggest sonification parameters via LLM reasoning.

        Asks the LLM to suggest audio synthesis settings (duration,
        volume, stereo spread) tailored to the observation and source
        characteristics. Falls back to hardcoded defaults if LLM
        response cannot be parsed.

        Args:
            metadata (ObservationMetadata): Observation metadata including
                target name, source count, object type, and exposure time
                for context.

        Returns:
            SonificationConfig: Duration, peak volume range, and stereo
                spread parameters. Falls back to defaults if LLM parse fails.
        """
        prompt = f"""You are a scientific sonification expert designing \
                audio representations of X-ray data.

                Observation: {metadata.target_name}
                Sources detected: {metadata.num_sources}
                Object Type: {metadata.object_type or 'unknown'}
                Exposure: {metadata.exposure_time}s

                Suggest sonification parameters:
                - duration: total playback in seconds (30-120 recommended)
                - peak_volume_range: [min, max] loudness ratio (e.g. [0.3, 1.0])
                - stereo_spread: [left, right] panning range (e.g. [0.3, 0.7])

                Consider:
                - Source density (more sources = longer duration to distinguish them)
                - Object type (galaxies might need slower sweeps, SNRs faster)
                - Listener comfort (avoid extreme panning or volume ranges)

                Respond with ONLY valid JSON:
                {{"duration": 60.0, "peak_volume_range": [0.3, 1.0], \
                "stereo_spread": [0.3, 0.7], "reasoning": "..."}}"""

        response = invoke_llm_with_retry(prompt)
        parsed = extract_json(response)

        peak_vol = parsed.get('peak_volume_range', list(
            FALLBACK_PEAK_VOLUME_RANGE))
        stereo = parsed.get('stereo_spread', list(FALLBACK_STEREO_SPREAD))

        try:
            return SonificationConfig(
                duration=float(parsed.get('duration', FALLBACK_DURATION)),
                peak_volume_range=(float(peak_vol[0]), float(peak_vol[1])),
                stereo_spread=(float(stereo[0]), float(stereo[1]))
            )
        except (ValidationError, IndexError, TypeError) as e:
            print(f"  LLM sonification config validation failed: {e}")
            return SonificationConfig(
                duration=FALLBACK_DURATION,
                peak_volume_range=FALLBACK_PEAK_VOLUME_RANGE,
                stereo_spread=FALLBACK_STEREO_SPREAD,
            )
    ...
    def suggest_processing_mode(
        ...
        """Ask LLM to select energy-band processing strategy.

        Uses observation metadata, object type, and source count to reason
        about whether full_band, dual_band, or triple_band processing is
        most scientifically appropriate.

        Args:
            metadata (ObservationMetadata): Observation metadata including
                target name, object type, and exposure time.
            num_sources (int): Number of detected X-ray sources.

        Returns:
            tuple[ProcessingMode, str]: Tuple of (selected mode, reasoning
                explanation). Falls back to FULL_BAND if parsing fails.
        """
        prompt = f"""You are an X-ray data analysis expert \
                deciding on energy band strategy.

                Observation: {metadata.target_name}
                Instrument: {metadata.instrument}
                Object Type: {metadata.object_type or 'unknown'}
                Sources Detected: {num_sources}
                Exposure: {metadata.exposure_time}s

                Choose the best processing strategy:
                - 'full_band'   — single 0.5-7 keV image (simple, good default) \
                (best for <{FALLBACK_DUAL_BAND_THRESHOLD} sources)
                - 'dual_band'   — soft (0.5-2 keV) + hard (2-7 keV) \
                (highlights spectral differences)
                - 'triple_band': Split soft (0.5-2 keV) + medium (2-4 keV) \
                + hard (4-7 keV) keV (best for spectral analysis)

                Consider:
                - Source density and crowding
                - Object type spectral properties (galaxies vs. SNRs vs. clusters)
                - Whether energy-dependent emission is scientifically important

                Choose based on source count, scientific interest,
                exposure depth.\n
                Use dual_band when ≥ {DUAL_BAND_THRESHOLD} sources and
                interesting spectral structure.\n
                Use triple_band only for very deep, scientifically rich fields.\n\n
                Respond with ONLY valid JSON:
                {{'mode': 'full_band', 'reasoning': '...'}}"""

        response = invoke_llm_with_retry(prompt)
        parsed = extract_json(response)
        mode_str = parsed.get('mode', 'full_band').lower()
        reasoning = parsed.get('reasoning', '')

        mode_map = {
            'full_band': ProcessingMode.FULL_BAND,
            'dual_band': ProcessingMode.DUAL_BAND,
            'triple_band': ProcessingMode.TRIPLE_BAND,
        }
        mode = mode_map.get(mode_str, ProcessingMode.FULL_BAND)
    ...
    def enrich_metadata_with_object_info(
        ...
        try:
            print(
                f"Looking up object information for "
                f"'{metadata.target_name}'...")
            ...
            if result is not None:
                ...
                redshifts = [float(row['rvz_redshift']) for row in result
            ...
            print(
                "  Warning: astroquery not installed, skipping object lookup")
            metadata.object_info = "Object lookup skipped" \
    ...
    def check_observation_exists(self, obs_id: str) -> dict[str, Any]:
        ...
        Returns a dict with last_step and files_present.
        """
        ...
        try:
            ...
            if result["log_entries"]:
                ...
                if "sonification_soft_config" in result["log_entries"] or \
                   "sonification_hard_config" in result["log_entries"]:
    ...
    def should_resume_observation(self, obs_id: str) -> tuple[bool, str]:
        ...
        print(f"\n{'='*60}")
        print("RESUMABLE STATE DETECTED")
        print(f"{'='*60}")
        print(f"Observation {obs_id} has been partially processed.")
        print(f"Last completed step: {obs_state['last_step']}")

        print("\nOptions:")
        print("  1. Resume from last completed step")
        print("  2. Restart from scratch (delete previous work)")
        print("  3. Cancel")
        ...
        while True:
            ...
            if choice == '1':
                ...
                confirm = input(
                    f"Delete all work in {obs_state['output_dir']}? "
                    "(y/n): ").strip().lower()
                if confirm == 'y':
    ...
    def decide_next_step(
        ...
        """
        Decide the next pipeline step based on the current step.
        Pure state-machine logic (no LLM needed here).
        """
        if current_step is None or current_step == "start":
            ...
            return {
                ...
                "reasoning":
                    "Full band detection done; select processing mode",
                "can_skip": False
            }
            ...
            return {
                ...
                "reasoning":
                    "Band processing complete; generate sonifications",
                "can_skip": False
            }
...
class SonificationCatalog:
    ...
    def scan_workspace(self) -> list[dict[str, Any]]:
        ...
        print(f"\n{'='*60}")
        print("SCANNING WORKSPACE FOR SONIFICATIONS")
        print(f"{'='*60}")
    ...
    def display_catalog(self) -> None:
        ...
        for entry in self.sonifications:
            ...
            print(f"{obs_id:<15} {created:<20} {audio_str:<15} "
    ...
    def show_sonification_details(self, obs_id: str) -> bool:
        ...
        if not any([entry['files']['audio_full'], entry['files']['audio_soft'],
    ...
    def interactive_browser(self) -> str | None:
        ...
        while True:
            ...
            if self.show_sonification_details(choice):
                ...
                try_again = input(
                    "Try another observation ID? (yes/no): ").strip().lower()
...
class FileSelection:
    ...
    def extract_obs_id(self, file_path: Path) -> str | None:
        ...
        if file_path.parent.name == 'secondary':
            ...
            if obs_id.isdigit():
                ...
                print(
                    f"Warning: Expected numeric observation ID, got: {obs_id}")
                return obs_id
    ...
    def select_evt2_file_with_obs_id_option(
        ...
        """
        Select evt2 file, with optional obs_id preference.
        If obs_id provided but not found, show menu with fallback options.
        """
        if requested_obs_id:
            evt2_file, obs_id = self.select_evt2_file_by_obs_id(
                requested_obs_id)
            if evt2_file:
            ...
            print(f"{'='*60}")
            print(f"Could not locate observation {requested_obs_id} "
                  "in workspace.")
            print("\nOptions:")
    ...
    def select_evt2_file(self) -> tuple[Path, str] | tuple[None, None]:
        ...
        try:
            ...
            print(
                "  Modified: "
                f"{datetime.fromtimestamp(latest.stat().st_mtime)}")
            ...
            print(f"Error: {e}")
            print(
                "\nNo evt2 files found automatically. "
    ...
    def _manual_file_selection(self) -> tuple[Path, str] | tuple[None, None]:
        ...
        while True:
            file_path = input(
                "\nEnter path to evt2 file (or 'cancel' to quit): ").strip()
            ...
            if 'evt2' not in path.name.lower():
                print(
                    f"Warning: Filename does not contain 'evt2': {path.name}")
                confirm = input("Continue anyway? (y/n): ").strip().lower()
                if confirm != 'y':
                    continue
...
class DataIngestion:
    """
    Deal with extracting correct data from FITS.
    """
    ...
    def load_source_list(self) -> Any:
        """Load source list from wavdetect output/src file."""
        with fits.open(self.src_path) as hdul:
            self.src = cast(Any, hdul["SRCLIST"]).data
        source_count = len(self.src) if self.src is not None else -1
...
class CIAOPreprocessing:
    ...
    def create_image(self,
        ...
        plt.title(
            f'X-ray Source Distribution ({len(src)} sources){title_suffix}')
        plt.tight_layout()
    ...
    def create_band_image(self, energy_spec: str = "500:7000",
                          band_name: str = "soft") -> str:
        """Create FITS image for specific energy band."""
...
class SourceDetection:
    """Using wavdetect to find sources."""
    ...
    def run_wavdetect(self, params: ProcessingParameters):
        ...
        if (Path(self.src_path).exists() and
                Path(self.psf_map_path).exists() and
                Path(self.nbkg_path).exists()):
        ...
        print(f"  Validity: {validity['n_passed']}/{validity['n_total']} "
...
class StraussSonification:
    ...
    Parameters are provided by the LLM at runtime.
    Fallback defaults are defined at the top of this file.
    """
    ...
    def __init__(self,
                 ...
                 peak_volume_range: tuple[float, float] = (
                     FALLBACK_PEAK_VOLUME_RANGE),
                 stereo_spread: tuple[float, float] = FALLBACK_STEREO_SPREAD,
                 note_len: float = FALLBACK_NOTE_LEN,
                 sf_preset: int = FALLBACK_SF_PRESET,
    ...
    def visualize_sources(self):
        ...
        plt.figure(figsize=(12, 8))
        scatter = plt.scatter(
            self.src_data["X"], self.src_data["Y"],
            s=(
                self.src_data["NET_COUNTS"]
                / self.src_data["NET_COUNTS"].max()
                * 200
            ),
            c=self.src_data["Y"],
            cmap='viridis',
    ...
    def prepare_objects(self):
        ...
        x_safe = np.clip(x_safe, 0, 1)
        # to ensure no values exceed 1 after scaling
        # else strauss will error out
        ...
        for i in range(self.objects.n_sources):
            ...
            t1 = min(t0 + self.note_len, self.duration)  # clamp t1 to duration
            # to avoid any potential issues with notes extending
        ...
        setattr(
            self.objects,
            'mapped_parameters',
            ['azimuth', 'pitch', 'volume', 'time']
        )

        print(f"  Created {self.objects.n_sources} objects "
    ...
    def create_sample(self):
        ...
        preferred = self.sf_preset \
            if self.sf_preset not in BROKEN_PRESETS else 2
    ...
    def render(self, save_path='xray_sonification.wav',
               master_volume=FALLBACK_MASTER_VOLUME):
...
class SonificationVisualizer(StraussSonification):
    """
    Extends StraussSonification with animated visualization
    of the scanning bar.
    """
    ...
    def __init__(self,
                 ...
                 sf_preset: int = FALLBACK_SF_PRESET,
                 x_bounds: tuple[float, float] | None = None):
        super().__init__(src_data, out_dir, duration,
                         peak_volume_range, stereo_spread, note_len, sf_preset,
                         x_bounds)
    ...
    def prepare_objects(self):
        ...
        self.counts_norm = ((self.src_data["NET_COUNTS"] -
                             self.src_data["NET_COUNTS"].min()) /
                            (self.src_data["NET_COUNTS"].max() -
                             self.src_data["NET_COUNTS"].min()))

    def visualize_with_animation(self,
        ...
        print(f"Animation timing: scan={scan_duration:.2f}s,"
        ...
        scatter = ax.scatter(
            self.src_data["X"], self.src_data["Y"],
            s=(self.src_data["NET_COUNTS"] /
                self.src_data["NET_COUNTS"].max()*200),
            c=self.src_data["Y"], cmap='viridis', alpha=0.6,
            edgecolors='black', linewidth=0.5, label='X-ray sources'
        )
        ...
        data_x_min, data_x_max = (
            self.src_data["X"].min(), self.src_data["X"].max())
        data_x_range = data_x_max - data_x_min

        scan_line = ax.axvline(
            x=data_x_min, color='red', linewidth=3,
            alpha=0.8, label='Scanning bar')
    ...
    def run_full_pipeline_with_viz(
            self,
            save_path='xray_sonification.wav',
            show_animation=True):
        ...
        if show_animation:
            ...
            try:
                ...
                if anim_result is None:
                    raise RuntimeError(
                        "Animation generation returned no result")
                fig, _ = anim_result
                plt.close(fig)
    ...
    def create_overlay_animation(
        ...
        if x_bounds is None:
            all_x = np.concatenate(
                [data[1]['X'] for data in src_data_dict.values()])
        ...
        try:
            anim.save(str(output_path), writer='ffmpeg', fps=fps,
                      extra_args=[
                        '-vcodec', VIDEO_CODEC, '-pix_fmt', 'yuv420p'])
    ...
    def create_overlay_video_with_mixed_audio(
            ...
            obs_id: str,
            duration: float = FALLBACK_DURATION) -> str | None:
        ...
        if len(wav_list) == 1:
            ...
            filter_complex = f'{filter_inputs}amix=' \
                f'inputs={len(wav_list)}:normalize=0[a]'
    ...
    @staticmethod
    def run_sonification_for_band(src_data,
                                  obs_id: str,
                                  band_name: str,
                                  band_label: str,
                                  duration: float = FALLBACK_DURATION,
    ...
    def create_combined_video(
        ...
        print(
            f"\nMixing {len(wav_paths)} audio track(s) "
            "and muxing with animation...")
        for p in wav_paths:
            print(f"  + {p}")
...
if __name__ == "__main__":
    print("="*60)
    print("X-RAY DATA PROCESSING PIPELINE (LLM-ASSISTED)")
    print("="*60)

    parser = argparse.ArgumentParser(
        description="LLM-assisted X-ray Data Processing Pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python llm_code.py                              # Interactive mode
  python llm_code.py --check                      # Browse sonifications
  python llm_code.py --check --check-id 932       # Open obs_id 932
  python llm_code.py -o 932                       # Process obs_id 932
  python llm_code.py -o 932 --resume              # Force resume (no menu)
  python llm_code.py -o 932 --fresh               # Force fresh start
  python llm_code.py -o 932 --band-mode dual      # Force dual band mode
  python llm_code.py --no-animation               # Skip visualizations
        """
    )
    ...
    parser.add_argument(
        '--fresh',
        action='store_true',
        help='Force fresh start (delete previous output_[obs_id]/ '
             'without asking)'
    )
    parser.add_argument(
        '-i', '--interactive',
    ...
    parser.add_argument(
        ...
        default=None,
        help='Force specific band mode (full/dual/triple) - skip LLM decision'
    )
    parser.add_argument(
        '--debug',
        action='store_true',
    ...
    evt2_file, obs_id = selector.select_evt2_file_with_obs_id_option(
        requested_obs_id)
    ...
    try:
        should_resume, resume_reason = (
            pipeline_manager.should_resume_observation(obs_id))
    ...
    print(f"File: {evt2_file}")

    # ============================================================
    # Step 2: Data Ingestion & Metadata Extraction
    # ============================================================
    if decision['next_step'] in ['metadata', 'full_band_detection',
                                 'band_selection', 'band_processing',
                                 'sonification', 'finished']:
        print("\n[2] Loading Metadata")
        print("="*60)
    ...
    # (LLM suggests detection parameters)
    # ============================================================
    ...
    preprocessing = CIAOPreprocessing(
        ...
        out_dir=f"output_{obs_id}/preprocessing"
    )

    # LLM suggests detection parameters (or load from saved run config)
    if run_config.replay and run_config.has("detection_params"):
        ...
        print("\n  Asking LLM for detection parameters...")
        detection_params = (
            pipeline_manager.suggest_detection_parameters(metadata))
        print(f"  LLM suggested: scales='{detection_params.wavdetect_scales}',"
              f" threshold={detection_params.significance_threshold}")
        print(f"  Reasoning: {detection_params.reasoning}")

        # Validate and adjust if LLM suggested something too strict
        run_config.log("detection_params", {
    ...
    try:
        full_band_image = preprocessing.create_band_image(
            energy_spec=ENERGY_BANDS["full"],
            band_name="full"
        )
        detector_full = SourceDetection(
            image_path=full_band_image,
            out_dir=f"output_{obs_id}/detection_full"
        )
        ...
        run_config.log("full_band_detection", {
    ...
    if num_sources == 0:
        ...
        print("The LLM may have suggested overly strict parameters.")

    # ============================================================
    # Step 4: Band Mode Selection (LLM decides)
    # ============================================================
    ...
    if run_config.replay and run_config.has("processing_mode"):
        ...
        if forced_band_mode:
            ...
            print("\n  Asking LLM for band strategy...")
            processing_mode, mode_reasoning = (
                pipeline_manager.suggest_processing_mode(
                    metadata, num_sources))
            print(f"  LLM chose: {processing_mode.value}")
            print(f"  Reasoning: {mode_reasoning}")

        run_config.log("processing_mode", processing_mode.value,
                       source="llm")

    # ============================================================
    # Step 5: Process bands
    # (LLM suggests sonification parameters per band)
    # ============================================================
    print("\n[5] Processing")
    print("="*60)
    ...
    if processing_mode == ProcessingMode.FULL_BAND:
        ...
        # LLM suggests sonification config
        ...
        if run_config.replay and run_config.has(soni_key):
            ...
            print("\n  Asking LLM for sonification parameters...")
            soni_config = (
                pipeline_manager.suggest_sonification_parameters(metadata))
            print(f"  LLM suggested: duration={soni_config.duration}s, "
                  f"volume={soni_config.peak_volume_range}, "
                  f"stereo={soni_config.stereo_spread}")
            run_config.log(soni_key, {
        ...
        result = SonificationVisualizer.run_sonification_for_band(
            ...
            band_label=BAND_LABELS["full"],
            duration=soni_config.duration,
            note_len=FALLBACK_NOTE_LEN,
            peak_volume_range=soni_config.peak_volume_range,
            stereo_spread=soni_config.stereo_spread,
            x_bounds=x_bounds,
        ...
        for band_name, (energy_spec, band_label) in bands.items():
            ...
            try:
                ...
                # LLM suggests sonification config per band
                ...
                if run_config.replay and run_config.has(soni_key):
                    ...
                    print(f"\n  Asking LLM for {band_name} "
                          "sonification parameters...")
                    soni_config = (
                        pipeline_manager.suggest_sonification_parameters(
                            metadata))
                    print(
                        f"  LLM suggested: duration={soni_config.duration}s, "
                        f"volume={soni_config.peak_volume_range}, "
                        f"stereo={soni_config.stereo_spread}")
                    run_config.log(soni_key, {
                        "duration":          soni_config.duration,
                        "peak_volume_range": list(
                            soni_config.peak_volume_range),
                        "stereo_spread":     list(
                            soni_config.stereo_spread),
                    }, source="llm")

                band_ingestion = DataIngestion(
                    evt2_path=str(evt2_file),
                    src_path=det_results['src_path'])
                ...
                if n == 0:
                    print(
                        "  Skipping sonification for "
                        f"{band_name} (no sources)")
                    continue
        ...
        if "soft" in sonification_results and "hard" in sonification_results:
            print("\n" + "="*60)
            create_mix = input(
                "Create stereo mix (soft=left ear, hard=right ear)? (y/n): "
            ).strip().lower()
            if create_mix == 'y':
                stereo_out = f"output_{obs_id}/sonification_stereo_mix.wav"
                ffmpeg_merge = [
                    'ffmpeg', '-y',
                    '-i', sonification_results['soft']['wav'],
                    '-i', sonification_results['hard']['wav'],
                ...
                try:
                    subprocess.run(
                        ffmpeg_merge, check=True, capture_output=True)
            ...
            create_overlay = input(
                ...
                "(both bands in one plot)? (y/n): ").strip().lower()
            if create_overlay == 'y':
                # Use the duration from the last soni_config
                overlay_duration = soni_config.duration
                try:
                    ...
                    animation_path = (
                        SonificationVisualizer.create_overlay_animation(
                    ...
                    if animation_path:
                        ...
                        final_overlay_video = (
                            ...
                            .create_overlay_video_with_mixed_audio(
